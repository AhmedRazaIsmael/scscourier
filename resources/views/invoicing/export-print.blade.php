...
<div class="invoice-container" style="margin-left: -10px;">
    ...
    <table style="width:100%;border-collapse:collapse;margin-bottom:0px; border:none">
      ...
      <p style="margin:2px 0;font-size:12px;">
        Website: <a href="{{ $invoice->company_website ?? '#' }}" style="color:#000;text-decoration:none;">
        {{ $invoice->company_website ?? 'www.airborncx.com' }}</a>
      </p>
    </td>
            ...
            @foreach($invoice->items as $index => $item)
                @php
                    if($item->account_head === 'Freight'){
                        ...
                        $cartons = $item->booking->pieces ?? '-'; // Use pieces as Cartons
                    } else {
                        // Keep existing logic: add amount to proper charge head
