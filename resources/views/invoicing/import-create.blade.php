...
<div class="app-container py-4">
    <form action="{{ route('invoice.import.store') }}" method="POST" enctype="multipart/form-data">
        @csrf

        <!-- Invoice Header -->
        <div class="card mb-4 shadow-sm">
            <div class="card-header">
                <strong><i class="bi bi-file-earmark-arrow-down"></i> Import Invoice</strong>
            </div>
            <div class="card-body">
                <div class="row g-3">
                    <div class="col-md-3">
...
function addRow(data={}) {
    ...
    // Book No cell: text input by default, select if customerBookings exist
...
// Attach input events for recalculation
...
// Fetch bookings when customer selected
document.getElementById('customer-select').addEventListener('change', function(){
    const customerId = this.value;
    if(!customerId) return;

    fetch(`/customer-import-bookings/${customerId}`)
        .then(res=>res.json())
        .then(data=>{
...
// Populate row when Book No is selected
function onBookSelect(sel){
    ...
    row.querySelector('input[name*="[gross_weight]"]').value = selected.weight || '';
